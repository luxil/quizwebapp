/**
 * hier werden die Fragen aus der Datenbank aufgerufen und an den Admin gesendet
 */
...
io.on('connection', function(socket) {
    socket.on('getQuestions', function (room) {
        var allQuestions;
        allQuestions = [];
            ...
            function (err, docs) {
                for (index = 0; index < docs.length; index++) {
                    ...
                    if (questionProcessed === docs.length) {
                        ...
                        socket.emit('showQuestions', {raum:room, allQuestions: allQuestions});
                    }
                }
            }
        );
